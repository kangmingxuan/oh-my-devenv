...
- **用本地 overlay，而非分叉** —— 把机器、团队与密钥设置放在 `$XDG_CONFIG_HOME/oh-my-devenv/` 和 `*.local` 文件中，绝不修改共享基线。配置根目录默认是 `~/.config`。
...
<details>
<summary><b>macOS：</b>在首次 apply 前添加本机专属 Homebrew 软件包</summary>

OrbStack 已属于所选的 macOS 桌面基线。若想在首次 apply 时一并安装其他本机专属 Homebrew 软件包，请在 bootstrap 运行前创建固定路径的本地 Brewfile：
...
系统包 hook 会在该文件存在时安装它。首次引导之后，本地 Brewfile 的变更需用 `brew bundle install --file="$XDG_CONFIG_HOME/oh-my-devenv/Brewfile.local"` 显式同步。
